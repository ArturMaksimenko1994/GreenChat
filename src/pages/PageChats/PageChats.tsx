...
import { getfetchContacts } from "@/api/api";
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      if (!idInstance || !apiTokenInstance) {
        console.error("Не все данные для запроса указаны.");
        return;
      }

      try {
        const data = await getfetchContacts(idInstance, apiTokenInstance);
        setContacts(data);
      } catch (error) {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.page__heading}>
        <h2 className={styles.page__title}>Чаты</h2>
      </div>
      <div className={`${styles.chats} custom-scrolling`}>
        {contacts.map((item) => (
          <Link className={styles.card} to={`/chats/${item.id}`} key={item.id}>
            ...
            <div className={styles.card__row}>
              ...
              <div className={styles.card__data}>
                <span className={styles.card__name}>{item.name || item.contactName || item.id}</span>
                <span className={styles.card__message}>сообщение</span>
              </div>
            </div>
          </Link>
        ))}
